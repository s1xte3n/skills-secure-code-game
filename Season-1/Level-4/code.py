...
class DB_CRUD_ops(object):
    ...
    def get_stock_info(self, stock_symbol):
        ...
        try:
            ...
            has_restricted_char = any([char in restricted_chars for char in stock_symbol])
    ...
    def get_stock_price(self, stock_symbol):
        ...
        try:
            ...
            dangerous_chars = ["'", ";", "--", " ", "\t", "\n"]
            sanitized_symbol = stock_symbol
            
            for char in dangerous_chars:
                if char in sanitized_symbol:
                    sanitized_symbol = sanitized_symbol.split(char)[0]
                    break

            res += "[QUERY] SELECT price FROM stocks WHERE symbol = '" + sanitized_symbol + "'\n"

            query = "SELECT price FROM stocks WHERE symbol = ?"
            
            cur.execute(query, (sanitized_symbol,))
            query_outcome = cur.fetchall()
            for result in query_outcome:
                res += "[RESULT] " + str(result) + "\n"
            return res
